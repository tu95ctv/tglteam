...
class Code128(Barcode):
    ...
    def _maybe_switch_charset(self, pos):
        ...
        if self._charset == 'C' and not char.isdigit():
            if char in list(code128.B.keys()):
                codes = self._new_charset('B')
            elif char in list(code128.A.keys()):
            ...
            elif char not in list(code128.B.keys()):
                if char in list(code128.A.keys()):
                    codes = self._new_charset('A')
        elif self._charset == 'A':
            if look_next():
                codes = self._new_charset('C')
            elif char not in list(code128.A.keys()):
                if char in list(code128.B.keys()):
                    codes = self._new_charset('B')
        return codes
    ...
    def _convert(self, char):
        if self._charset == 'A':
            ...
            if char in list(code128.C.keys()):
