...
class SummaryReportConfigLine(models.Model):
    ...
    field_id = fields.Many2one('ir.model.fields', domain="[('model_id','=',model_id), ('store', '=', True), ('ttype', 'in', ('integer', 'float', 'monetary'))]", required=True, string='Aggregate Field')
    method = fields.Selection([('sum', 'Sum'), ('min', 'Min'), ('max', 'Max'), ('avg', 'Average'), ('count', 'Count')], default='sum', required=True)
    ...
    digits = fields.Integer(default=0)
    currency_id = fields.Many2one('res.currency')
    sequence = fields.Integer()

    date_range_field_id = fields.Many2one('ir.model.fields', domain="[('store', '=', True), ('model_id','=',model_id), ('ttype','in',('date', 'datetime'))]")
    ...
    def _compute_html_table(self):
        for r in self:
            ...
            if summary_report_config.template_id:
                ...
                template_id = self.env.ref('web_banner_configuration.tb_21_Minimal_Statistics_With_Gradient_Background_Color__New_Posts')
