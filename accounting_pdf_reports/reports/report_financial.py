...
class ReportFinancial(models.AbstractModel):
            ...
            if report.type == 'accounts':
                ...
                for value in res[report.id]['account'].values():
                    for field in fields:
                        res[report.id][field] += value.get(field)
            # TGL
            elif report.type == 'account_filtered':
                # it's the sum the leaf accounts with such an account type
                accounts = self.env['account.account'].search(safe_eval(report.domain))
                res[report.id]['account'] = self._compute_account_balance(accounts)
                for value in res[report.id]['account'].values():
                    for field in fields:
                        res[report.id][field] += value.get(field)
    ...
    def get_account_lines(self, data):
        ...
        for report in child_reports:
            vals = {
                ...
                # TGL
                'code': report.code,
                'right_header': report.right_header,
    ...
    def _get_report_values(self, docids, data=None):
        ...
        return {
            ...
            'right_header': report_lines[0].get('right_header'),
            'full_name': report_lines[0].get('full_name'),
        }
