...
class AccountFinancialReport(models.Model):
    ...
    type = fields.Selection([
        ...
        ('account_filtered', 'Filter accounts'),
        ], 'Type', default='sum')
    ...
    domain = fields.Char('Filter accounts')
    right_header = fields.Html('Right header')
    full_name = fields.Html('Full name')

    @api.model
    def create(self, values):
        if values.get('right_header'):
            right_header = values.get('right_header')
            if right_header.find('<p') >= 0:
                right_header = right_header.replace('<p', '<div')
                right_header = right_header.replace('</p', '</div')
                values['right_header'] = right_header

        return super(AccountFinancialReport, self).create(values)
    ...
    def write(self, values):
        ...
        return super(AccountFinancialReport, self).write(values)
